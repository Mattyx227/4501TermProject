...
onready var hurtbox = $DemonHitBox
...
func _process(delta):
	...
	match current_state:
		...
		State.CHASE:
			...
			sprite.play("Walk")
			sprite.flip_h = velocity.x < 0
			$DemonHitBox/CollisionShape2D.position = $DemonHitBox/CollisionShape2D.position * Vector2(-1, -1)
	...
	if sprite.flip_h == true:
		hurtbox.get_node("CollisionShape2D").position = Vector2(-10, 1)
	else:
		hurtbox.get_node("CollisionShape2D").position = Vector2(4, 1)
